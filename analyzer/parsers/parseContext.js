  ...
  try {
    ...
    traverse(ast, {
      ...
      ExportDefaultDeclaration({ node }) {
        if (node.declaration.type === 'ObjectExpression') {
          // 检查是否有install方法
          const hasInstall = node.declaration.properties.some(prop => 
            prop.key && prop.key.name === 'install'
          );
          if (hasInstall) {
      ...
      // 在install函数体内扫描require.context调用
    ...
    for (const contextCall of contextCalls) {
      ...
      if (result.type === 'vue-install') {
        ...
        const files = scanContextFiles(contextPath, contextCall.recursive, contextCall.regExp);
...
/**
 * 获取文件名（不含扩展名），类似FileUtil.name的实现
...
function scanContextFiles(contextPath, recursive, regExpStr) {
  ...
  try {
    ...
    const pattern = regExpMatch ? regExpMatch[1] : regExpStr.replace(/[\/]/g, '');
    ...
    function scanDir(dir, currentDepth = 0) {
      ...
      for (const entry of entries) {
        ...
        if (stat.isDirectory() && recursive) {
          ...
          const relativePath = './' + path.relative(contextPath, fullPath).replace(/\\/g, '/');
